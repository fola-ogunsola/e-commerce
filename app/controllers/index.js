...
function createAdmin(req,res) {
    ...
    var confirmation_code = OTP;
    const value = [first_name, last_name, email, phone_number, hash, confirmation_code]
    db.any(queries.createAdmin, value)
    .then(function(data){
        nodemailer.sendEmailConfirmation(
            first_name,
            email,
...
function adminLogin( req,res){
    ...
    .then(function(result){
        ...
        .then(function(data){
            if(!data){
                return res.json({status: 'false', message: 'Email and password do not match'})
            }else{
                var token = jwt.sign({email: data.email}, auth.secret,{
                    expiresIn: 86400
                })
...
function verifyAdmin(req, res){
    const value = [req.params.confirmation_code, req.body.email]
    console.log(req.params.confirmation_code)
    db.oneOrNone(queries.verifyAdmin, value)
    .then(function(admin){
        if(!admin){
            return res.status(404).send({message: "Admin Not found."})
        }
        else {
            return res.json({status: "success", message:"Account Confirmed Successfully"})
        }
    })
    .catch(function(err){
        return res.json({status: "failed", message: "Unable to verify admin"})
    })
}
...
module.exports = {createUser, verifyUser, userLogin, createAdmin, adminLogin, verifyAdmin,}